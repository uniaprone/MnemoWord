...
public interface WordDao {
    ...
    @Query("SELECT DISTINCT w.id " +
            ...
            "   LEFT JOIN word_review r ON r.id = w.id " +
            "   LEFT JOIN day_plan_word dpw ON dpw.word_id = w.id AND dpw.date = :date" +
            "   WHERE r.id IS NULL AND dpw.word_id IS NULL AND w.id NOT IN (:excludeWordIds)" +
            "   ORDER BY RANDOM() " +
            "   LIMIT :newCount")
    List<Long> addTodayNewLearningWordEntities(int newCount, String date, List<Long> excludeWordIds);

    @Query("SELECT w.id " +
            "   FROM words w " +
            "   INNER JOIN word_review r ON r.id = w.id " +
            "   WHERE r.next_review_time IS NOT NULL" +
            "   AND r.next_review_time <= :today" +
            "   AND r.review_status < 2")
    List<Long> selectTodayReviewingWordEntities(String today);
