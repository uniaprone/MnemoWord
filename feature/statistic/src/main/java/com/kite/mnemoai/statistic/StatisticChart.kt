...
class StatisticChart(val chart: CombinedChart, val studyStatistics: List<StudyStatistic>) {
    ...
    private val axisGridColor: Int = MaterialColors.getColor(chart, R.attr.colorSurfaceContainerLow)
    ...
    init{
        ...
        studyStatistics.forEachIndexed { index, statistic ->
            ...
            stackedEntries.add(
                BarEntry(
                    index + 0.5f,
                    floatArrayOf(
                        statistic.newLearnedCount.toFloat(),
                        statistic.reviewedCount.toFloat()
                    )
                )
            )
        ...
        val barDataSet = BarDataSet(stackedEntries, "").apply {
            ...
            setColors(
                MaterialColors.getColor(chart, android.R.attr.colorPrimary),
                MaterialColors.getColor(chart, android.R.attr.colorSecondary)
            )
        }
        ...
        chart.apply {
            ...
            setDrawOrder(
                arrayOf<CombinedChart.DrawOrder>(
